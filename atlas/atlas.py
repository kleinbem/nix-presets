    ...
    def get_short_lived_token(engine: AtlasEngine):
        ...
        try:
            ...
            if not all([app_id, install_id, priv_key]):
                log(
                    "GitHub App Identity not found in SOPS. Falling back to PAT...",
                    YELLOW,
                )
            ...
            log(
                "Auth libraries not found. Run via 'nix run .#atlas' for the full environment.",
                RED,
            )
            return None
...
def cmd_status(engine: AtlasEngine):
    ...
    for name, cfg in sorted(containers.items()):
        ...
        if name in ["ollama", "n8n", "agent-team"]:
            tag = f"{MAGENTA}AI/WORK{NC}"
        elif cfg.get("enable", False):
            tag = f"{BLUE}CORE{NC}"
        else:
            tag = f"{NC}WORKLOAD{NC}"

        endpoint = f"https://{name}.local" if active == "active" else "---"
        print(
            f"{color}{name:<18}{NC} | {color}{status:<10}{NC} | {tag:<21} | {endpoint}"
        )
    print(
        f"{BLUE}{BOLD}================================================================${NC}\n"
    )
...
def main():
    ...
    parser = argparse.ArgumentParser(
        prog="atlas", description="Sanctuary Workspace Toolkit"
    )
    ...
    launch.add_argument(
        "server", choices=["github", "brave-search", "filesystem", "atlas"]
    )
    ...
    if args.command == "status":
        cmd_status(engine)
    elif args.command == "mcp" and args.sub == "launch":
        cmd_mcp_launch(engine, args.server, args.exec)
    else:
        parser.print_help()
