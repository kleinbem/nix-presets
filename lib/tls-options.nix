  ...
  tls = {
    ...
    upstreams = lib.mkOption {
      type = lib.types.listOf (lib.types.submodule {
        options = {
          target = lib.mkOption { type = lib.types.str; description = "IP of the upstream container."; };
          port = lib.mkOption { type = lib.types.int; default = 443; description = "Port on the upstream (usually 443 for mTLS)."; };
        };
      });
      default = [ ];
